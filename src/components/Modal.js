...
export default function BasicModal({item }) {
  ...
  return (
    <div className=''>
      <Button onClick={handleOpen} className='modal-btn'>More details...</Button>
      <Modal
        open={open}
        onClose={handleClose}
      ...
      >
       ...
       <Typography id="modal-modal-title" variant="h6" component="h2">
          <h1 className='modal-category'>{item.category}</h1 >
          </Typography>
         <Typography id="modal-modal-title" variant="h6" component="h2">
...
{cart.some((x) => x.id === item.id) ? (
   <p onClick={() => removeItemFromCart(item)} className='modal-btn-cart'>
     Remove from cart
     <ShoppingCartIcon style={{ width: 15 }} />
   </p>
 ) : (
   <p onClick={() => addItemToCart(item)} className='modal-add-cart'>
     Add to cart
     <ShoppingCartIcon style={{ width: 15 }} />
   </p>
 )}
